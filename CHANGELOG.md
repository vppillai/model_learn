...
- Task 8: GGUF conversion (f16/Q8_0/Q4_K_M) + Ollama run + Labs 07/08. **Phase 1 complete** — the hand-built model runs on CPU via `ollama run tinystories-slm`. Gotchas: llama.cpp build OOM (-j 4), custom-tokenizer hash registration, llama-cli `-st`, Ollama needs zstd + absolute FROM path.
